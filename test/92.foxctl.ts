...
describe('92.foxctl', function (this: any) {
  ...
  it('8.1a schema registered via foxctl rejects a mismatched publish at ingest time', async () => {
    // Regression test: AdminApiServer used to construct its own SchemaRepository
    // instance per call, disconnected from the one BaseRealm's SchemaChecker
    // validates publishes against. That let malformed payloads land in
    // event_history unvalidated, only to fail much later during KV activation.
    const numericSchema = {
      properties: { id: 'string', credit: 'number' },
      primary_key: ['id'],
      url_pattern: 'billing.*'
    }
    const schemaFile = writeSchemaFile(numericSchema)
    try {
      const addOut = await ctl('schema', 'add', 'billing', schemaFile)
      expect(addOut.status, addOut.stderr).to.equal(0)
    } finally {
      fs.unlinkSync(schemaFile)
    }
    ...
    try {
      await api.publish('billing.1', { id: '1', credit: 'not-a-number' }, {
        retain: true, trace: true, acknowledge: true
      })
    } catch (e) {
      publishError = e
    }
    expect(publishError).to.not.be.null
    expect(publishError.message).to.include('Field "credit" expected type "number"')

    const history = await db.all(`SELECT * FROM event_history_${REALM}`)
    expect(history).to.have.lengthOf(0)
  })

  it('8.1b kv add registers a projection bound to a schema; kv list shows it inactive', async () => {
    const schemaFile = writeSchemaFile(CUSTOMER_SCHEMA)
    let schemaId: string
    try {
      const addOut = await ctl('schema', 'add', 'customer', schemaFile)
      schemaId = addOut.stdout.match(/Registered: (\S+)/)![1]
    } finally {
      fs.unlinkSync(schemaFile)
    }

    const kvAddOut = await ctl('kv', 'add', 'cust-add', schemaId)
    expect(kvAddOut.status, kvAddOut.stderr).to.equal(0)
    expect(kvAddOut.stdout).to.match(/Registered: cust-add/)
    expect(kvAddOut.stdout).to.include('customer.*')

    const listOut = await ctl('--json', 'kv', 'list')
    const storages = parseJsonOutput(listOut.stdout)
    const proj = storages.find((s: any) => s.name === 'cust-add')
    expect(proj).to.exist
    expect(proj.status).to.equal('inactive')
    expect(proj.schemaId).to.equal(schemaId)
  })

  it('8.1c kv add with unknown schema id fails on stderr, exit 1', async () => {
    const out = await ctl('kv', 'add', 'ghost-proj', 'sch_does_not_exist')
    expect(out.status).to.equal(1)
    expect(out.stderr).to.include('Schema not found')
  })
