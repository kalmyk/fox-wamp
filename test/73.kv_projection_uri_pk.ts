...
describe('73.kv_projection_uri_pk', function () {
  ...
  it('applies an MQTT-originated event whose history body is p64-wrapped', async () => {
    // Retained MQTT publishes are stored as { payload: Buffer }, which
    // makeDataSerializable() turns into { p64: base64 } before it lands in
    // event_history (see lib/realm.ts). Backfill/live projection must unwrap
    // that with unSerializeData() before reading the payload out.
    const schemaJson = {
      properties: { user_id: 'string', username: 'string' },
      primary_key: ['user_id']
    }
    ...
    const projection = new KvProjection(db, record, schemaJson, dataTable)
    const p64 = Buffer.from(JSON.stringify({ user_id: '123', username: 'jdoe' })).toString('base64')
