...
describe('56.kv_registry', function () {
  ...
  describe('ProjectionListener', function () {
    ...
    it('5.8 activation skips a schema-nonconforming event, warns, and still completes online', async () => {
      // Retained event missing primary key 'id' — fails schema validation. A schema
      // may be registered after older, nonconforming events already exist, so this
      // must not abort the whole projection — it's skipped with a warning instead.
      ...
      expect(rec!.skippedCount).to.equal(1)
      expect(rec!.lastWarning).to.be.a('string')
      expect(rec!.lastWarning).to.include('id')
      // The skipped event still counts as processed for backfill progress.
      expect(rec!.currentPosition).to.equal('seg1a1')
    })
    ...
    it('5.18 schema validation runs before projected value is stored; nonconforming live events are skipped, not failed', async () => {
      await listener.activateProjection('realm1', PROJ)
      ...
      // No data stored due to validation failure, but the projection keeps running
      const rows = await db.all(`SELECT * FROM "${schemaRec.dataTable}"`)
      expect(rows).to.have.lengthOf(0)
    })
