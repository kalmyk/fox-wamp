...
function Session (gateHandler, sender, sessionId) {
  ...
  */
  ...
  this.setSimultaneousTaskLimit = function (aLimit) {
    simultaneousTaskLimit = aLimit
  }
  ...
  this.isAble = function () {
    return (simultaneousTaskLimit < 0) || (simultaneousTaskLimit - tasksRequested) > 0
  }
  ...
  this.checkWaitTask = function (engine) {
    ...
    if (this.isAble()) {
      for (let [key, subD] of sSub) {
        ...
        if (!this.isAble()) {
          return found // exit if worker got enough tasks
        }
      }
    }
    return found
