...
class EntrySession {
  constructor (wampSession, syncMass, history) {
    ...
    wampSession.subscribe('storeInHistory', (args, kwargs, opts) => {
      console.log("storeInHistory EVENT", args, kwargs);
    })

    wampSession.subscribe('storeOutHistory', (args, kwargs, opts) => {
  ...
  sendToSync (applicantId) {
    ...
    for (let [,ss] of this.syncMass) {
      ss.publish('mkSegmentId', [], {applicantId})
    }
