...
const { keyId, ProduceId } = require("./makeid")
const History = require('../sqlite/history')
const { getDbFactoryInstance } = require('../sqlite/dbfactory')
...
class SessionEntryHistory extends EventEmitter {
  constructor (wampSession, syncMass, gateId, pubResult) {
    super()
    ...
    wampSession.subscribe('begin-advance-segment', (args, kwargs, opts) => {
      ...
      wampSession.publish('trim-advance-segment', [{advanceSegment: advanceSegment}])
    })
  ...
  dbSaveSegment (segment, segmentId) {
    let result = []
    const makeId = new ProduceId((a) => a)
    makeId.actualizePrefix(segmentId.dt + keyId(segmentId.id))

    for (let row of segment.content) {
      let id = makeId.generateIdStr()
      History.saveEventHistory(getDbFactoryInstance().getMainDb(), row.realm, id, row.uri, row.data, row.opt)
