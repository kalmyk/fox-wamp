...
export class NetEngine extends BaseEngine {
  ...
  constructor (netEngineMill: NetEngineMill) {
    ...
    // Derived from whether any storage node has ever announced itself — see NetSubStatusFactory.
    // Defined via defineProperty (rather than a class accessor) because BaseEngine declares
    // this as a plain field, and TS forbids overriding a field with an accessor (TS2611).
    Object.defineProperty(this, 'supportsRetainedEventSync', {
      configurable: true,
      enumerable: true,
      get: () => this.netEngineMill.getNetSubStatusFactory().hasStorageNodes()
    })
  }
  ...
  getHistoryAfter (after: string, uri: string[], cbEmitRow: any): Promise<any> {
    ...
    const buffer = this.netEngineMill.getOrCreateBuffer(realm)
    buffer.ensureLoading(this.netEngineMill.getSysApi(), realm, after)
    return buffer.drainUntil(after, uri, cbEmitRow)
  }
}
...
export class NetEngineMill extends EventEmitter {
  ...
  constructor (router: Router, configQuorum: number) {
    ...
    this.netSubStatusFactory = new NetSubStatusFactory(this.sysApi)
  ...
  getSysApi (): HyperClient {
    return this.sysApi
  }

  getNetSubStatusFactory (): NetSubStatusFactory {
    return this.netSubStatusFactory
  }

  getOrCreateBuffer (realm: string): SharedSegmentBuffer {
    let buffer = this.realmBuffers.get(realm)
    if (!buffer) {
      buffer = new SharedSegmentBuffer()
      this.realmBuffers.set(realm, buffer)
    }
    return buffer
  }
