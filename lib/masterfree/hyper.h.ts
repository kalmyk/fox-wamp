...
export namespace Event {
  ...
  export const historyFetchTopic = (shardTag: number) =>
    `fox.storage.history.fetch.${shardTag}`
}
...
export type BODY_STORAGE_NODE_CONNECTED = {
  nodeId: string
}
...
export type HistoryFetchProgress = {
  events: HistoryEvent[]
  lastEventId: string
}
...
export namespace AdminEvent {
  ...
  export const KV_ADD = 'fox.admin.kv.add'
...
export type AdminKvAddRequest = { name: string; schemaId: string }
export type AdminKvAddResponse = { name: string; schemaId: string; uriPattern: string; status: StorageStatus }
