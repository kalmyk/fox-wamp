...
class WampHandler extends BaseGate {
  hello (ctx, session, realmName, details) {
    ...
    } else if (typeof this._authHandler.authTicket === 'function' && details.authmethods.indexOf('ticket') >= 0) {
  ...
  checkAuthorize (session, funcClass, uri) {
    if (this.isAuthorizeRequired() && !this._authHandler.authorize(session, funcClass, uri)) {
...
handlers[WAMP.REGISTER] = function (ctx, session, message) {
  ...
  this.checkAuthorize(session, 'register', uri)
...
handlers[WAMP.CALL] = function (ctx, session, message) {
  ...
  this.checkRealm(session, id)
  this.checkAuthorize(session, 'call', uri)
...
handlers[WAMP.SUBSCRIBE] = function (ctx, session, message) {
  ...
  this.checkRealm(session, id)
  this.checkAuthorize(session, 'subscribe', uri)
...
handlers[WAMP.PUBLISH] = function (ctx, session, message) {
  ...
  this.checkRealm(session, id)
  this.checkAuthorize(session, 'publish', uri)
