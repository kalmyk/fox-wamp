    ...
    this.hello = function(session, message) {
        ...
        if (this.isAuthRequired()) {
            ...
            router.getRealm(session.realmName, function (realm) {
                ...
                this.sendWelcome(session.sender);
            }.bind(this));
        }
    }

    this.sendWelcome = function (sender) {
        sender.send({ returnCode: 0, sessionPresent: false, cmd: 'connack' });
    };
    ...
    this.handle = function (session, msg) {
        ...
        try {
            ...
            if (err instanceof RealmError) {
                this.sendError(session, mtype, err.requestId, err.message, err.args);
            }
            else {
                throw err;
            }
