...
function mapStorageRecord(realmName: string, row: any): StorageRecord {
  return {
    ...
    skippedCount: row.skipped_count === null || row.skipped_count === undefined ? 0 : row.skipped_count,
    lastWarning: row.last_warning === undefined ? null : row.last_warning,
  }
}
...
export class StorageRegistry {
  ...
  async get(name: string): Promise<StorageRecord | null> {
    ...
    const row = await this.db.get(
      `SELECT name, schema_id, uri_pattern, started_at, status, current_position, last_error, skipped_count, last_warning
        FROM ${this.tableName}
        WHERE name = ?`,
      [name]
    )
  ...
  // projection. Not routed through update_history — this can fire once per
