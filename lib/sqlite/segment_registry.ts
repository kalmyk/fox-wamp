...
export async function listResolvedSegmentIdsForShard (db: sqlite.Database, realmName: string, shardTag: number): Promise<string[]> {
  const rows = await db.all(
    `SELECT segment_id FROM ${tableName(realmName)} WHERE shard_tag = ? AND status = 'resolved' ORDER BY segment_id ASC`,
    [shardTag]
  )
