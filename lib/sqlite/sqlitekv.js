...
class SqliteModKv {
  ...
  setKeyValue (realmName, suri, origin, data, opt, sid, pubOutEvent) {
    this.pkq.enQueue(realmName + '|' + suri, async () => {
      ...
      try {
        ...
        const newData = deepDataMerge(oldData ? oldData.value : null, data)
        ...
        if (isDataEmpty(newData)) {
          ...
          await this.db.run(
            ...
            [realmName, suri, JSON.stringify(data), willSid, JSON.stringify(opt)]
          )
        }
        this.saveUpdateHistory(origin, origin, realmName, suri, oldData, newData)
  ...
  getKey (realmName, uri, cbRow) {
    ...
    return this.pkq.enQueue(realmName + '|' + strUri, async () => {
      await this.db.each(
        'SELECT key, value, opt, stamp FROM kv WHERE kv_realm = ? AND key = ?',
        [realmName, strUri],
        (err, row) => {
          cbRow(row.key, JSON.parse(row.value), row.stamp)
        }
      )
    })
  }
  ...
  saveUpdateHistory (id, origin, realmName, suri, oldv, newv) {
    return this.db.run(
      ...
      [id, origin, realmName, suri, JSON.stringify(oldv), JSON.stringify(newv)]
    )
  }
}

class SqliteKv extends KeyValueStorageAbstract {
  constructor (mod, realmName) {
    super()
    this.mod = mod
