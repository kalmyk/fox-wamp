...
function Api(router, realm) {
    ...
    this.regrpc = function(uri, callback) {
        let regId = realm.regrpc(this, tools.randomId(), uri);
        if (regId) {
            _rpc[regId] = callback;
        }
        return regId;
...
function Realm(router, realmName) {
    ...
    this.callrpc = function(session, callId, procUri, options, args, kwargs) {
        ...
        if (destSession) {
            ...
            _pending.set(invId, [callId, session.sessionId]);
            var invOpts = {};
    ...
    this.resrpc = function(session, invId, err, args, kwargs, options) {
        ...
        if (_pending.has(invId)) {
            let invocation = _pending.get(invId);
            let destSession = router.getSession(invocation[1]);
            if (destSession) {
                destSession.sendResult(invocation[0], err, args, kwargs, resOpts);
            }
        }
        if (!resOpts.progress) {
            _pending.delete(invId);
        }
    };
    ...
    this.substopic = function(session, requestId, topicUri, options) {
        ...
        let subscription = {topicId:topicId, topicUri:topicUri, sessionId:session.sessionId};
        ...
        _topics.add(topicUri, subscription);

        router.emit('Subscribed', this, topicUri);
        session.sendSubscribed(requestId, topicId);
        _storage.getKey(topicUri).then(
            (value) => {
                if (value) {
                    let requestId = value[0];
                    let args = value[2];
                    let kwArgs = value[3];
                    session.sendEvent(topicId, requestId, args, kwArgs, {});
                }
            },
            (reason) => {}
        );
    ...
    this.unsubstopic = function(session, requestId, topicId) {
        ...
        if (_sessTopic.hasOwnProperty(session.sessionId) && _sessTopic[session.sessionId].hasOwnProperty(topicId)) {
            let subscription = _sessTopic[session.sessionId][topicId];
    ...
    this.publish = function(session, requestId, topicUri, options, args, kwargs) {
        ...
        if (options && options.hasOwnProperty('retain')) {
            let sessionId = 0;
            if (options.hasOwnProperty('weak')) {
              sessionId = session.sessionId;
            }
            _storage.addKey(topicUri, publicationId, sessionId, args, kwargs);
        }

        let found = _topics.match(topicUri);
        let eventOpts = {topic:topicUri};
        for (let i = 0; i < found.length; i++) {
            let destSession = router.getSession(found[i].sessionId);
            if (destSession) {
                ...
                _topics.remove(topicUri, found[i]);
            }
        }
...
//        return publicationId; // result not used
    };
