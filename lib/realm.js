...
class Actor {
    ...
    reject(errorCode, message)
    {
        ...
        session.gate.sendError(session.sender, {
            id:this.msg.id,
            data:data
        });
    };
    ...
    acknowledged() {
        let session = this.realm.getSession(this.sid);
        session.gate.acknowledged(session.sender, this.msg);
    }
}
...
class ActorCall extends Actor {
    ...
    getData(){
        return {args:this.msg.args, kwargs:this.msg.kwargs}
    }
    ...
    responseArrived(actor)
    {
        ...
        let session = this.realm.getSession(this.sid);
        session.gate.sendResult(session.sender, {
            id:this.msg.id,
            err:actor.msg.err,
            data:actor.msg.data,
            pgs:actor.msg.pgs,
...
class ActorReg extends Actor
{
    callWorker(taskD)
    {
        ...
        let session = this.realm.getSession(this.sid);
        session.gate.sendInvoke(session.sender, {
            id:taskD.markId,
            qid:this.subId,
            uri:taskD.queueId,
            myd:this.msg.id,
...
class ActorPush extends Actor
{
    responseArrived(actor) {
        if (!this.clientNotified)
        {
            ...
            var ack = this.msg.opt && this.msg.opt.acknowledge;
            if (ack) {
                this.acknowledged();
            }
...
class EngineBase {
    ...
    doneDefer(sessionId, markId)
    {
        ...
        if (found && found.destSessionId.hasOwnProperty(sessionId))
        {
            ...
            console.log('defer not found', markId, this.defers);
//            throw {Message:"internal error: id "+markId+" not found in settle"};
        }
    };
}
...
class EngineRpc {
    ...
    getSubStack(queueId) {
      return (
        this.wSub.hasOwnProperty(queueId) ?
        this.wSub[queueId] :
          {}
      );
    };
  
    waitForResolver(queueId, header)
    {
      if (!this.qCall.hasOwnProperty(queueId))
      this.qCall[queueId] = [];
  
      this.qCall[queueId].push(header);
    };
    ...
    checkTasks(subD)
    {
      if (this.qCall.hasOwnProperty(subD.queueId)) {
        var taskD = this.qCall[subD.queueId].shift();
        if (this.qCall[subD.queueId].length === 0)
          delete this.qCall[subD.queueId];
    ...
    doCall(actor) {
      let queue = this.getSubStack(actor.queueId);
      let subExists = false;
      for(var index in queue) {
        let subD = queue[index];
        subExists = true;
        if (subD.isAble()) {
          subD.callWorker(actor);
          return null;
        }
      }
      ...
      if (!subExists)
      {
        throw new RealmError(actor.msg.id,
            ...
            ['no callee registered for procedure <'+actor.msg.uri+'>']
        );
      }

      // all workers are bisy, keep the message till to the one of them free
      this.waitForResolver(actor.queueId, actor);
    }
}
...
class EnginePush {
    ...
    doPush(actor) {
        ...
        let found = this.match(actor.queueId);
...
function BaseRealm(router, realmName, base, rpc, push) {
    var _api = null;
    ...
    this.unregrpc = function(session, cmd) {
        let registration = session.removeSub(this.rpc, cmd.qid);
        if (registration)
        {
    ...
    this.callrpc = function(session, cmd) {
        ...
        actor.markId = this.base.makeDeferId();
        actor.queueId = cmd.uri;
        this.rpc.doCall(actor);
        return actor.markId;
    }

    this.resrpc = function(session, cmd) {
        let invocation = this.base.getDefer(session.sessionId, cmd.id);
        if (invocation) {
    ...
    this.unsubstopic = function(session, cmd) {
        let subscription = session.removeTrace(this.push, cmd.qid);
        if (subscription)
        {
    ...
    this.cleanupSession = function(session) {
        ...
        session.cleanupTrace(this);
        session.cleanupReg(this);
