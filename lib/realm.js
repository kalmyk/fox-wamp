...
const { match, intersect, merge, extract, defaultParse, restoreUri } = require('./topic_pattern')
...
class Actor {
  ...
  acknowledged () {
    this.ctx.acknowledged(this.msg)
  }
...
class BaseEngine {
  ...
  setKey (uri, sessionId, data, messageId) {
    for (let i = this.kvo.length - 1; i >= 0; i--) {
      ...
      if (match(uri, kvr.uri)) {
        kvr.kv.setKey(extract(uri,kvr.uri), sessionId, data, messageId)
        break
      }
    }
  }
  ...
  removeSession (sessionId) {
    for (let i = this.kvo.length - 1; i >= 0; i--) {
      this.kvo[i].kv.removeSession(sessionId)
    }
  }
  ...
  getKey (cbRow, cbDone, uri) {
    for (let i = this.kvo.length - 1; i >= 0; i--) {
      const kvr = this.kvo[i]
      if (match(uri, kvr.uri)) { // ?? intersect
        // console.log('MATCH++', uri, kvr.uri);
        kvr.kv.getKey((aKey, data, messageId) => {
          // console.log('GOT', aKey, data);
          cbRow(merge(aKey, kvr.uri), data, messageId)
        }, cbDone, extract(uri,kvr.uri))
        return
      }
    }
    cbDone()
...
class BaseRealm extends EventEmitter {
  ...
  doEcho (ctx, cmd) {
    let a = this.createActorEcho(ctx, cmd)
    a.acknowledged()
  }
  ...
  doTrace (ctx, cmd) {
    const session = ctx.getSession()
    let subscription = this.createActorTrace(ctx, cmd)
  ...
  registerKeyValueEngine (uri, kv) {
    this.engine.registerKeyValueEngine(uri, kv)
  }
...
class KeyValueStorageAbstract {
  // getKey (cbRow, cbDone, uri) ::result:: aKey, data, messageId
  // removeSession (sessionId)
}
...
class Realm extends BaseRealm {
  constructor (router, realmName) {
    ...
    this._wampApi = null
    this.registerKeyValueEngine(['#'], new MemKeyValueStorage())
  }
