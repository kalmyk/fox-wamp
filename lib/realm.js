...
class BaseEngine {
  ...
  cleanupSession(sessionId) {
    let result = []
    for (let i = this._kvo.length - 1; i >= 0; i--) {
      result.push(this._kvo[i].kv.removeSession(sessionId))
    }
...
class BaseRealm extends EventEmitter {
  ...
  makeInboundEvent (uri, data) {
    return this.engine.doPush(new ActorPushKv(
      uri,
      data,
      { retain: true, trace: true }
    ))
  }

  registerKeyValueEngine (uri, kv) {
    kv.setUriPattern(uri)
    kv.setKeepUpdateHistory(this.engine.keepUpdateHistory.bind(this.engine))
    kv.setMakeInboundEvent(this.makeInboundEvent.bind(this))
...
class KeyValueStorageAbstract {
  ...
  setKeepUpdateHistory(keepUpdateHistory) {
    this.keepUpdateHistory = keepUpdateHistory
  }

  setMakeInboundEvent(makeInboundEvent) {
    this.makeInboundEvent = makeInboundEvent
  }
