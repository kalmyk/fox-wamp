...
class ActorTrace extends Actor {
  ...
  filter (event) {
    if (this.retained && !event.opt.retained) {
      return false
    }
    if (!this.retained && event.opt.retained) {
      return false
...
function deepDataMerge (to, from) {
  for (let n in from) {
    if (typeof to[n] != 'object') {
      to[n] = from[n]
    } else if (typeof from[n] == 'object') {
      to[n] = deepDataMerge(to[n], from[n])
    }
  }
  return to
}
...
class BaseEngine {
  ...
  doPush (actor) {
    if (actor.getOpt().retain) {
      this.setValueFromActor(actor)
    } else {
      this.keepHistory(actor)
    }
  }

  setValueFromActor (actor) {
  ...
  registerKeyValueEngine (uri, kv) {
    ...
    kv.keepHistory = this.keepHistory.bind(this)
...
class BaseRealm extends EventEmitter {
  ...
  setKeyData (uri, data) {
    return this.engine.setKeyData(uri, data)
  }
