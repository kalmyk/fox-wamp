...
class ActorCall extends Actor {
  constructor (session, msg) {
    super(session, msg)
    this.engine = session.realm.engine
  }
...
class BaseEngine {
  ...
  checkTasks (subD) {
    let strUri = restoreUri(subD.getUri())
  ...
  doCall (taskD) {
    let strUri = restoreUri(taskD.getUri())
    let queue = this.getSubStack(strUri)
    let subExists = false
...
class BaseRealm {
  ...
  createActorEcho  (session, cmd) { return new Actor      (session, cmd) }
  createActorReg   (session, cmd) { return new ActorReg   (session, cmd) }
  createActorCall  (session, cmd) { return new ActorCall  (session, cmd) }
  createActorYield (session, cmd) { return new ActorYield (session, cmd) }
  createActorTrace (session, cmd) { return new ActorTrace (session, cmd) }
  createActorPush  (session, cmd) { return new ActorPush  (session, cmd) }

  doEcho (session, cmd) {
    let a = this.createActorEcho(session, cmd)
    a.acknowledged()
  }
  ...
  doRegRpc (session, cmd) {
    ...
    // }
    let actor = this.createActorReg(session, cmd)
    ...
    if (actor.getOpt().reducer /* || actor.getOpt().transformTo */) {
      ...
      this._router.emit(ON_SUBSCRIBED, this, actor)
    } else {
      actor.acknowledged()
    }
  ...
  doUnRegRpc (session, cmd) {
    let registration = session.removeSub(this.engine, cmd.unr)
    if (registration) {
  ...
  doCallRpc (session, cmd) {
    let actor = this.createActorCall(session, cmd)
  ...
  doYield (session, cmd) {
    let invocation = this.engine.qYield.getDefer(session.sessionId, cmd.qid)
    if (invocation) {
      invocation.responseArrived(this.createActorYield(session, cmd))
    } else {
  ...
  doConfirm (session, cmd) {
    let defer = this.engine.qConf.getDefer(session.sessionId, cmd.qid)
    if (defer) {
  ...
  // Topic Management
  doTrace (session, cmd) {
    let subscription = this.createActorTrace(session, cmd)
  ...
  doUnTrace (session, cmd) {
    let subscription = session.removeTrace(this.engine, cmd.unr)
    if (subscription) {
  ...
  doPush (session, cmd) {
    let actor = this.createActorPush(session, cmd)
    ...
    if (cmd.opt && cmd.opt.hasOwnProperty('retain')) {
      ...
      this.engine.setKey(cmd.uri, sessionId, cmd.data, actor.eventId)
    }
  }
  ...
  cleanupSession (session) {
    ...
    this.engine.removeSession(session.sessionId)
...
class MemEngine extends BaseEngine {
  ...
  setKey (uri, sessionId, data, messageId) {
    if (data === null) {
      ...
      this._keyDb.set(restoreUri(uri), [sessionId, data, messageId])
    }
  }
  ...
  removeSession (sessionId) {
    ...
    for (let key in this._keyDb) {
      let keySessionId = this._keyDb.get(key)[0]
