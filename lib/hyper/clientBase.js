...
Task.prototype.resolve = function(result) {
  if (!this.isFinished) {
    ...
    this.taskResponseCallback(this.cmd, RESULT_OK, { kv:result });
    this.cmd = null;
  }
};
...
Task.prototype.reject = function(reason) {
  if (!this.isFinished) {
    this.isFinished = true;
    this.taskResponseCallback(this.cmd, RESULT_ERR, { kv: reason });
    this.cmd = null;
  }
};
...
Task.prototype.notify = function(data) {
  if (!this.isFinished) {
    this.taskResponseCallback(this.cmd, RESULT_EMIT, { kv: data });
  }
};
