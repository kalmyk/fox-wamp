...
class HyperApiContext extends Context {
  ...
  sendInvoke (cmd) {
    const opt = Object.assign({}, cmd.opt)
    if (cmd.hdr) {
      opt.headers = cmd.hdr
    }
    try {
      const taskResult = cmd.id.cb(cmd.data, opt)
      Promise.resolve(taskResult).then(result => 
        this._realm.cmdYield(this, {
          rqt: RESULT_OK,
          qid: cmd.qid,
  ...
  sendResult (cmd) {
    ...
    if (cmd.err) {
      ...
      cmd.id.resolve(getBodyValue(cmd.data))
    }
  }

  sendEvent (cmd) {
    cmd.id.cb(getBodyValue(cmd.data), {sid: cmd.sid, publication: cmd.qid})
  }
...
function HyperSocketFormatter (socket) {
  ...
  this.settle = function (action, cmd) {
    ...
    case RESULT_OK:     action.resolve(getBodyValue(cmd.data)); return true
    case RESULT_ERR:    action.reject (cmd.data); return true
    case RESULT_EMIT:   action.cb(cmd.data); return false  
    case REQUEST_EVENT: { // =========================================
      try {
        action.cb(getBodyValue(cmd.data), {topic: restoreUri(cmd.uri)})
      }
      catch (e) {
