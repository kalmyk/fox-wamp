...
function HyperClient (realm, ctx) {
  ...
  this.pipe = (writeToClient, topic, pipeOpt) => {
    return this.subscribe(topic, (args, opt) => {
      const newOpt = Object.assign({}, pipeOpt)
      if (opt && opt.headers) {
        newOpt.headers = opt.headers
      }
      writeToClient.publish(opt.topic, args, newOpt)
    })
  }
...
function RemoteHyperClient (formater) {
  ...
  const atOpenCallbacks = []

  this.onopen = (callback) => {
    if (typeof callback === 'function') {
      atOpenCallbacks.push(callback)
    } else {
      throw new Error('onopen callback must be a function')
    }
  }

  this.applyOnOpen = async () => {
    for (const callback of atOpenCallbacks) {
      await callback()
    }
  }
