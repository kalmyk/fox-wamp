      ...
      if (isJson()) {
        ...
        printTable(
          storages.map((s: any) => ({
            ...
            last_warning: s.lastWarning ?? '',
          })),
          ['name', 'status', 'current_position', 'last_error', 'skipped', 'last_warning']
        )
      }
    }))

  kv.command('add <name> <schema-id>')
    .description('register a KV projection bound to a schema (pattern taken from the schema)')
    .action((name: string, schemaId: string) => runCommand(async (client) => {
      const result: any = await client.callrpc(AdminEvent.KV_ADD, { name, schemaId })
      console.log(`Registered: ${result.name} (schema ${result.schemaId}, pattern ${result.uriPattern}) → ${result.status}`)
      console.log(`Activate with: foxctl --realm ${getRealm()} kv activate ${result.name}`)
    }))
      ...
      runCommand(async (client) => {
        ...
        if (waitFlag) {
          ...
          while (status === 'refreshing') {
            ...
            rec = (listResult.storages || []).find((s: any) => s.name === name)
            status = rec ? rec.status : 'unknown'
          }
          if (status === 'online') {
            console.log(`${name} is online`)
            if (rec?.skippedCount > 0) {
              console.log(`Warning: ${rec.skippedCount} event(s) skipped (schema mismatch). Last: ${rec.lastWarning}`)
            }
          } else {
...
if (!group || group === '--help' || group === '-h') {
  ...
  console.log('  kv add <name> <schema-id>      register a KV projection bound to a schema')
