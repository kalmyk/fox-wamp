# retained-state-event-sync Specification

## Purpose
Allows subscribers to specify an `after` event ID on SUBSCRIBE so that retained state delivery is delayed until the engine has committed that event to retained storage. Solves the publish-then-subscribe race condition for local engines (in-memory and SQLite DbEngine). Distributed support is gated on KV projection watermark observability.
