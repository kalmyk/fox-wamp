...
- [x] 2.3 Add `findShardsForNode(nodeId)` method returning `{ shards }` or `null`
...
- [x] 4.1 Implement `getDestinationTopics()` on the segment object to return `[keepHistoryShardTopic(shardTag)]` when `sharded=true`, else `[Event.KEEP_ADVANCE_HISTORY]`
- [x] 4.2 Pass `sharded: boolean` (true when `eventNodes` is present in config) into `NetEngineMill`
...
- [x] 5.3 Subscribe to `keepHistory.<bucket>` for each owned bucket instead of the broadcast `Event.KEEP_ADVANCE_HISTORY`
...
- [x] 8.3 Unit test: `getDestinationTopics()` returns `keepHistory.<shardTag>` when `sharded=true` (e.g. shardTag=5 → `keepHistory.5`), broadcast when `sharded=false`
- [x] 8.4 Unit test: `findShardsForNode` returns correct shards; node subscribes to matching `keepHistory.*` topics
...
- [x] 9.2 Register `fox.admin.event.shard.list` handler in `AdminApiServer`: reads `eventNodes` from config and returns `{ shardCount, shards: [{ bucket, nodeId, host, port }] }`
