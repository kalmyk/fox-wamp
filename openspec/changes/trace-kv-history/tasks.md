...
- [ ] 1.2 Ensure the table includes `msg_id` (PK), `msg_origin`, `msg_uri`, and `msg_oldv`.
...
- [ ] 3.3 Ensure schema identifiers are used consistently in the `msg_uri` field.

## 4. Verification

- [ ] 4.1 Add unit tests for `saveUpdateHistory` to verify correct SQL execution and data serialization.
- [ ] 4.2 Add integration tests for KV updates verifying history entries are created with correct origin IDs.
